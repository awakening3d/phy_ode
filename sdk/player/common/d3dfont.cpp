...
bool CD3DFont::CreateBitmapBits()
{
	DeleteBitmapBits();
	...
	//dmOutToConsole("create bitmapbits: %d", m_pBitmapBits);

	if (!m_pBitmapBits) return false;
	...
	for (TCHAR c = 32; c<127; c++) {
		str[0] = c;
		GetTextExtentPoint32(m_hDC, str, 1, &size);
		...
		ExtTextOut(m_hDC, x + 0, y + 0, ETO_OPAQUE, NULL, str, 1, NULL);
...
void CD3DFont::DeleteBitmapBits()
{
	if (m_hbmBitmap) DeleteObject(m_hbmBitmap), m_hbmBitmap = NULL;
...
HRESULT CD3DFont::RestoreDeviceObjects()
{
	...
	if (!m_pBitmapBits) CreateBitmapBits();
	...
	// Create a system texture for lock
	LPDIRECT3DTEXTURE9 ptexsys = NULL;
	hr = m_pd3dDevice->CreateTexture(m_dwTexWidth, m_dwTexHeight, 1,
		0, D3DFMT_A4R4G4B4,
		D3DPOOL_SYSTEMMEM, &ptexsys, NULL);
	if (!ptexsys) {
	...
	hr = ptexsys->LockRect(0, &d3dlr, 0, 0);
	if (D3D_OK == hr) {
		if (m_pBitmapBits) {
			BYTE* pDstRow = (BYTE*)d3dlr.pBits;
			WORD* pDst16;
			BYTE bAlpha = 0; // 4-bit measure of pixel intensity
	...
	m_pd3dDevice->UpdateTexture(ptexsys, m_pTexture);
                                  ...
                                  TCHAR* strText, DWORD dwFlags )
{
	...
	if (m_dwDrawTextStateBlock) m_dwDrawTextStateBlock->Apply();
...
// Desc: Draws 2D text
//-----------------------------------------------------------------------------
HRESULT CD3DFont::DrawText( FLOAT sx, FLOAT sy, TCHAR* strText, DWORD dwColor, DWORD dwFlags )                    
{
	//g_Draw.TextOut( sx, sy, strText, dwColor );
//	return S_OK;
...
//-----------------------------------------------------------------------------
HRESULT CD3DFont::Render3DText(const Vec3& vOrg, const Vec3& vWidthDir, const Vec3& vHeightDir, TCHAR* strText,  DWORD dwFlags )
{
    if( m_pd3dDevice == NULL )
        return E_FAIL;
    ...
    while( c = *strText++ )
    {
        ...
        if( c != _T(' ') )
        {
            *pVertices++ = InitFont3DVertex( TO3D(x+0,y+0), D3DXVECTOR3(0,0,-1), tx1, ty1 );
			*pVertices++ = InitFont3DVertex( TO3D(x+w,y+0), D3DXVECTOR3(0,0,-1), tx2, ty1);
			*pVertices++ = InitFont3DVertex( TO3D(x+0,y+h), D3DXVECTOR3(0,0,-1), tx1, ty2 );

            *pVertices++ = InitFont3DVertex( TO3D(x+w,y+h), D3DXVECTOR3(0,0,-1), tx2, ty2 );
            *pVertices++ = InitFont3DVertex( TO3D(x+0,y+h), D3DXVECTOR3(0,0,-1), tx1, ty2 );
			*pVertices++ = InitFont3DVertex( TO3D(x+w,y+0), D3DXVECTOR3(0,0,-1), tx2, ty1);
            dwNumTriangles += 2;
